...
import { RoleParams, set_role } from '@/store/slice/role';
...
const LoginPage: React.FC = () => {
    ...
    const onLogin = async (values: RoleParams) => {
        const res: AxiosRes = await axios.post('/api/user/login', values)
    ...
    return <div className={style.page_container}>
        ...
        <div className={style.login_container}>
            <div className={style.login_left}>
                <div className={style.left_title}><img src={login_desc} alt="" /></div>
            </div>
            <div className={style.login_right}>
                <div className={style.right_title}>
                    <div><img src={login_logo} alt="" /></div>
                    <div className={style.title_container}>
                        <div><img src={login_title_cn} alt="" /></div>
                        <div><img src={login_title_en} alt="" /></div>
                    </div>
                </div>
                <div className={style.right_form}>
                    ...
                    >
                        <Form.Item
                            label="手机号"
                            name="phone"
                            rules={[{ required: true, message: '请填写手机号' }, { pattern: new RegExp(/^1(3|4|5|6|7|8|9)\d{9}$/, "g"), message: '请输入正确的手机号' }]}
                        >
                        ...
                        <div style={{ 'position': 'relative' }}>
                            ...
                            <div className={[style.form_code_btn, count !== 0 ? style.form_code_btn_disabled : ''].join(' ')} onClick={startCode}>{count === 0 ? '获取验证码' : `${count}秒`}</div>
                        </div>
